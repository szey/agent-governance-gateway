...
网页管理的批准清单保存在 Git 忽略的 `data/approved-agents.json`，可能包含 Agent 名称、路径片段、负责人和内部批准单号，必须留在获准的数据边界内。管理 API 使用专用 Header 降低跨站误操作风险，但这不是身份认证或 RBAC。批准清单只处理资产准入，绝不绕过逐次行为策略和审计。
...
本地 Server 默认只监听 `127.0.0.1`。当前批准清单管理功能只适合本机控制台；如果要对局域网或公网开放，必须另行评审身份认证、CSRF、防重放、TLS、防火墙和访问控制设计，当前 MVP 不提供这些保护。
