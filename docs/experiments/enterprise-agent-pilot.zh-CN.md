...
状态：**探索性试用后暂停，正式受控试点未完成**。一次公司端点试用验证了服务启动和限定目录配置发现，同时暴露了扫描中断、目录/市场噪声、网页不刷新和批准清单缺少入口的问题；本轮已用 synthetic fixture 和本地测试推动修复。没有把原始公司日志、用户名、主机名或绝对路径保存在仓库。后续继续执行前仍必须重新获得设备负责人、IT/安全团队和相关数据负责人的明确授权。
...
| 观察 | 结论 | 本轮产品响应 |
|---|---|---|
| 健康检查成功，限定目录扫描得到 Agent/MCP 证据 | 配置发现路径可运行，但不是运行行为证明 | 保留证据等级和覆盖限制 |
| 扫描整个系统盘在无权限目录处中断 | 全盘扫描既不合规也不可靠 | 仍要求限定目录；子目录拒绝访问改为 coverage gap 并继续 |
| marketplace、插件 cache 和临时内容大量命中 | “可获得”不等于“已安装或已使用” | 新增 `available/installed/configured/observed` 状态；仅 deployed 状态参与 Shadow 核对 |
| CLI 扫描结果没有进入已运行网页 | 启动时快照不符合实际使用流程 | 网页增加“重新扫描已配置目录”，批准清单变化后自动核对 |
| 没有可见的批准清单入口 | 无法解释为什么结果是 Shadow | 网页增加本地批准清单；支持负责人、批准单号、到期日和暂停状态 |
| “已批准”容易被误解为“所有动作均获准” | 资产准入与行为授权必须分离 | 界面明确 `Approved Agent ≠ approved behavior`；所有经过 Router 的有效行为继续审计 |

这次试用没有完成真实 GUI/IDE Agent 行为观测、独立 OS/网络佐证或性能测量，因此不能升级为 `V3 pilot_verified`。
...
- GitHub 源码仓库可以是 Public，但“公开可下载”不代表“公司批准运行”；
- 首次传入公司前运行 secret scan，并让公司确认是否允许从个人 GitHub 拉取公开源码；
